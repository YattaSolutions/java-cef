...
class CefBrowserWr extends CefBrowser_N {
	...
	private void doUpdate() {
		try {
			CefApp.getGuiHandler().syncExec(() -> {
				if (isClosed())
					return;

				Rectangle vr = ((JPanel) component_).getVisibleRect();
				Rectangle clipping = new Rectangle((int) (vr.getX() * scaleFactor_), (int) (vr.getY() * scaleFactor_),
						(int) (vr.getWidth() * scaleFactor_), (int) (vr.getHeight() * scaleFactor_));
				...
				if (OS.isMacintosh()) {
					...
					synchronized (content_rect_) {
						...
						content_rect_ = new Rectangle((int) (bounds.getX() * scaleFactor_),
								(int) (bounds.getY() * scaleFactor_), (int) (bounds.getWidth() * scaleFactor_),
